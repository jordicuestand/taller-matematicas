...
Nos ha quedado una [EDO lineal de 2º orden](/taller-matematicas/2015/04/26/edos-lineales-de-orden-superior/) en la función y(x), cuya solución general es $y=C_1\cos(x)+C_2\sin(x)+4x+3$. Sustituyendo en la primera de las ecuaciones del sistema:
...
Vemos que la última ecuación, $y^{\left(4\right)}+5y''+2\left(-y''-3y\right)=4e^{2x}\Leftrightarrow y^{\left(4\right)}+3y''-6y=4e^{2x}$, sólo contiene la función y(x), y es lineal de coeficientes constantes de cuarto orden, que podremos resolver con los métodos de [EDOs lineales de orden superior](/taller-matematicas/2015/04/26/edos-lineales-de-orden-superior/). Una vez obtenida y(x), la sustituimos en la ecuación $z''+y'-2z=e^{2x}$ para obtener z(x).
...
que se resuelven con los [métodos del operador D para EDOs lineales](/taller-matematicas/2015/05/01/metodos-operacionales-el-operador-d/).
...
y resulta una ecuación lineal también homogénea para y: $\left(P_1\left(D\right)P_4\left(D\right)-P_2\left(D\right)P_3\left(D\right)\right)y=0.$ Es sabido (ver por ejemplo [EDOs lineales de orden superior](/taller-matematicas/2015/04/26/edos-lineales-de-orden-superior/)) que las soluciones de las ecuación lineales  homogéneas de coeficientes constantes son siempre del tipo $Ce^{rx}$, por tanto se puede probar directamente con funciones de este tipo, sustituyendo en el sistema. Además, las ecuaciones que resultan para las otras incógnitas son las mismas, para z(x) será:
...
Para encontrar una solución particular de un sistema lineal de primer orden se pueden generalizar los métodos usados en[ EDOs lineales de primer orden](/taller-matematicas/2015/03/28/ecuaciones-diferenciales-lineales-de-1r-orden/): los métodos de coeficientes indeterminados y variación de constantes. Aquí sólo emplearemos el primero.
