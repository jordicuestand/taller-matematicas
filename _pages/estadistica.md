---
layout: page
title: "Estadística"
date: 2015-04-14 12:44:23 +0000
math: true
---
{% raw %}

**Estadística teórica (modelos matemáticos)**

 	- [Combinatoria](http://tallermatematic.eu/wp/?p=1449)

 	- [Problemas de combinatoria](http://tallermatematic.eu/wp/?p=1311)

 	- [Problemas de probabilidades]({{ '/2016/10/16/problemas-de-probabilidades/' | relative_url }})

 	- [Problemas de distribuciones continuas de probabilidad]({{ '/2015/05/27/problemas-de-distribuciones-de-probabilidad-continua/' | relative_url }})

 	- [Variables aleatórias]({{ '/2017/01/07/variables-aleatorias/' | relative_url }})

 	- [Problemas de estimación de parámetros](http://tallermatematic.eu/wp/?p=1464)

 

**Estadística aplicada**

 	- [Introducción a la Estadística y al análisis de datos; Estadística descriptiva](http://tallermatematic.eu/wp/?p=1341)

 	- [Introducción a las probabilidades](http://tallermatematic.eu/wp/?p=1389)

 	- [Series temporales]({{ '/2016/11/27/estadistica-estadistica-aplicada-series-temporales/' | relative_url }})
{% endraw %}